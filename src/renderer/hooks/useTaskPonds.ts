...
import { getPondList, reorderPonds } from "@/renderer/api/pond";
import {
  getTaskList,
  addTask,
  editTask,
  ...
  reorderTasks,
  delTask,
} from "@/renderer/api/task";
import { ITaskResult, IPondResult } from "@/renderer/types/task";
import { getHistoryList } from "@/renderer/api/history";
...
export const useAddTask = (queryKey: string) => {
  ...
  return useMutation(
    ({ belong, userId, describe }: any) =>
      addTask({ belong, userId, describe }),
    {
...
export const useDelTask = (queryKey: string) => {
  const queryClient = useQueryClient();
  return useMutation(
    (params: { id: number; sort: number }) => delTask(params),
    {
      onSuccess: () => queryClient.invalidateQueries(queryKey),
    }
  );
...
export const useReorderTask = (queryKey: string) => {
  ...
  return useMutation(
    (data: ITaskSortProps) => reorderTasks<ITaskSortProps>(data),
    {
      // 查询成功后的回调
      onSuccess: () => queryClient.invalidateQueries(queryKey),
      async onMutate(target) {
        ...
        queryClient.setQueryData(queryKey, (old: any) => {
          const { fromId, referenceId, fromPondId, toPondId, type, tag } =
            target;
